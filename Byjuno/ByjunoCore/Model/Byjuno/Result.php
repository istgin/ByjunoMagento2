...
class Result {
    ...
    public function handle(array $_vars)
    {
        ...
        $order->loadByIncrementId(($_vars['OrderID'] == "DUMMY") ? $_vars['Reference'] : $_vars['OrderID'])
                ->addStatusHistoryComment(sprintf(Mage::helper("byjunocore")->__("Customer returned with status: %s"), $_vars['StatusCode']))
                ->save();
        ...
        switch (strtoupper($_vars['Status'])) {
            case "ERR":                
                ...
                $msg = sprintf(Mage::helper("byjunocore")->__("The payment provider has returned the following error message: %s"), Mage::helper("byjunocore")->__($_vars['Status'] . ": " . $_vars['StatusCode']));
                $url = 'checkout/cart';
